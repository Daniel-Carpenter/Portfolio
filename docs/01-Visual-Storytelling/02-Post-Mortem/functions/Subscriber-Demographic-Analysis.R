...
# ==============================================================================
# Function that gets new member (subscriber) signups data by demographic bin. Plots trends + Pre/Post Comparison
# Returns: Data frame with daily subscriber signups by age bin, drive time bin, or gender. Depends on inputs
...
subscriber_demographic_analysis <- function(
  ...
  df_monthly_signups_demo <- df_new_subscriber_demo |>
  ...
  df_overview_plot <- df_monthly_signups_demo |>

    # Group by month by indirectly not groupung by date, also re count the signups by month
    group_by(across(c(-date_of_trx, -num_signups, -DEMOGRAPHIC_VALUE))) |>
    summarise(num_signups = sum(num_signups)) |>
    mutate(num_signups_R12_AVG = rollmean(num_signups, MONTHS_TO_AVERAGE,na.pad=TRUE, align="right")) |>
    drop_na()
  ...
  # Create the # signups label for the segmentation plot (not rolling average)
  label_most_recent_month_signup <- geom_label_repel(data=label_Data,

                                                     # Create the label
                                                     aes(label=paste0(
                                                       format(month_end_date, '%b-%y'),       # Apr-22
                                                       ':\nSignups: ', comma(round(num_signups,-2))), # Signups: 123
                                                       y=num_signups+0.10*num_signups),     # Set the y axis coordinate
                                                     # nudge_x =-30,
  ...
  # Create the # signups label for the segmentation plot (rolling average)
  createR12Label <- function(df, includePadding=TRUE) {
    label <- geom_label_repel(data=df,
                              ...
                              aes(label=paste0(ifelse(includePadding, 'R12 avg.\n', 'R12 avg: '), # R12 avg: 123
                                               comma(round(num_signups_R12_AVG, -2), accuracy=1)),
                                  y=num_signups_R12_AVG + max(df_overview_plot$num_signups_R12_AVG) * 0.2), # Set the y axis coordinate
  ...
  basePlot.overview <- df_overview_plot |> filter(month_end_date <= last_full_mo) |>
    ...
    theme_get() +
    labs(title    = 'Number of New Subscriber Signups Decrease After Competitor Entry',
         subtitle = paste0(''),
  ...
  plot_overview <- basePlot.overview +
    ...
    geom_col(aes(y  = num_signups, fill = 'Number of Signups'), alpha=0.4) +
    scale_fill_manual(values = c("Number of Signups" = paste0(getFillCol('blue')))) + # Legend

    # Line of rolling 12 mo. avg. number of signups
    geom_line(aes(y = num_signups_R12_AVG, color = 'R12 avg. Num. Signups'), alpha=0.9, lwd=1) +
    scale_color_manual(values = c("R12 avg. Num. Signups" = paste0(getLineCol('blue')))) + # Legend
  ...
  df_segment_pie <- df_monthly_signups_demo |>
    group_by(DEMOGRAPHIC_VALUE) |>
    summarise(num_signups = sum(num_signups)) |>
    drop_na()

  # Calculate the percentage of each group
  df_segment_pie$percent_num_signups = df_segment_pie$num_signups / sum(df_segment_pie$num_signups)
  ...
  plot_segment_pie <- ggplot(df_segment_pie,
    ...
    theme(legend.position = 'top',
          ...
          plot_caption = element_text(hjust = 0.5)) +
  ...
  df_segment_plot_data <- df_monthly_signups_demo |>

    # Group by month by indirectly not groupung by date, also re count the signups by month
    group_by(across(c(-date_of_trx, -num_signups))) |>
    summarise(num_signups = sum(num_signups)) |>
    mutate(num_signups_R12_AVG = rollmean(num_signups, MONTHS_TO_AVERAGE, na.pad=TRUE, align="right")) |>
    drop_na()
  ...
  basePlot.segment <- df_segment_plot_data |>
    ...
    facet_grid(rows   = vars(DEMOGRAPHIC_VALUE),
               scales = 'free_y') +
    labs(title    = paste0('Number of New Subscriber Signups by ', demographicName, '\n'),
         subtitle = paste0(''),
         x='Month of Year', y='Num. New Subscribers') +
    scale_y_continuous(labels = comma)
  ...
  # ----------------------------------------------------------------------------
  # Daily Average Comparison - Cumulative Signups vs. 12-Mo. avg. Trend
  # ----------------------------------------------------------------------------
  ...
  df_avgBase <- df_new_subscriber_demo |>
    ...
    group_by(across(c(-DEMOGRAPHIC, -DEMOGRAPHIC_VALUE, -num_signups))) |>
    summarise(num_signups = sum(num_signups)) |>
  ...
  df_weekday_avg <- df_avgBase |>
    ...
    # Get the average signups per weekday by store
    group_by(across(c(-date_of_trx, -num_signups))) |>
    summarise(AVG_num_signups = round(mean(num_signups)))


  # Change before and after comparison date ------------------------------------
  df_avg_signups_comparison_detail <- df_avgBase |>
  ...
  df_avg_signups_comparison_trend <- df_avg_signups_comparison_detail |>
    ...
    mutate(cum_sum_actual = cumsum(num_signups),
           cum_sum_avg    = cumsum(AVG_num_signups),
  ...
  legend.labels = c(paste0(num_prior_months_to_avg, '-mo. Weekday avg. Signups'),
                    'Post-Competitor Entering Market Signups')

  PRE_COMPARISON_COLOR  = 'grey87' # Color for the comparison

  plot_signups_cumulative_var <- df_avg_signups_comparison_trend |>
    ...
    annotate("text", x = max(df_avg_signups_comparison_trend$date_of_trx) - days(4), y = 0.03, label = "Historical Baseline",
             color = getLineCol('charcoal')) +
    ...
    labs(title    = paste0('Daily Signups Since Competitor Entering Market Drop Below ', num_prior_months_to_avg,
                           '-Month Weekday Average\n' ),
         subtitle = paste0('See Caption for Calculation\n'),
         x = 'Date', y = '% Change | Cumulative Signups vs. 12 mo. avg. Signups',
         caption = paste0('\nCalculation: (Cumulative Signups - Cumulative 12-mo. avg Signups by Weekday) / Cumulative 12-mo. avg Signups by Weekday.\n',
                          '"Baseline" refers to 12-mo. avg. Signups by Weekday.'
         )
    ) +
    theme_get()
  # plot_signups_cumulative_var

  # Plot daily actuals vs. historical signups by DOW ---------------------------

  SPACING_AMT = 0.15

  # Internal scatter of daily (facet day of week). Grey out prior and highlight current
  plot_signups_detail <- df_avg_signups_comparison_detail |>
    ...
    # Daily signups for each point, jitter amount ensures little overlap for viewing ease
    ...
    labs(title    = 'Daily Signups Since Competitor Entering Market Drop Below Historical Average\n',
         subtitle = paste0('Comparison Data from Rolling ',num_prior_months_to_avg, '-Months\n'),
         x = 'Day of Week', y = 'Number of Daily Signups',
  ...
  # Daily Comparison
  # dashboard_dailySignups <- plot_grid(plot_singupsTrend, plot_signups_detail,
  ...
  output <- list('data'  = df_new_subscriber_demo,
                 'plots' = list('overview'     = dashboard_overview,
                                ...
                                'cumulativeVariance' = plot_signups_cumulative_var,
                                'detail'       = plot_signups_detail))

  return(output)
}

# signups <- subscriber_demographic_analysis(
#   selected_stores = c(1),
